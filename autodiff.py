...
class Node(object):
    ...
    def __mul__(self, other):
        """TODO: Your code here (Finished by Junyi)"""
        if isinstance(other, Node):
...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here (Finished by Junyi)"""
        assert len(input_vals) == 2
        input_vals[0] = input_vals[0] if not node.matmul_attr_trans_A else input_vals[0].T
        input_vals[1] = input_vals[1] if not node.matmul_attr_trans_B else input_vals[1].T
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if isinstance(node.op, PlaceholderOp):
                continue
            input_vals = [node_to_val_map[i] for i in node.inputs]
